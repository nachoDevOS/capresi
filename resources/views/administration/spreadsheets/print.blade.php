        ...
        <tr>
            ...
            <td style="text-align: center;  width:50%">
                ...
                <h4 style="margin-bottom: 0px; margin-top: 5px">
                    PLANILLA DE PAGO DE {{ strtoupper($monthList[$spreadsheet->month] )}} DE {{ date('Y', strtotime($spreadsheet->year)) }}
            ...
            <td style="text-align: right; width:30%">
                <h3 style="margin-bottom: 0px; margin-top: 5px">
                    <div id="qr_code">
                        {!! QrCode::size(80)->generate('Planilla de pago '.strtoupper($monthList[$spreadsheet->month]).' de '.date('Y', strtotime($spreadsheet->year ))); !!}
                    </div>
    ...
    <table style="width: 100%; font-size: 10px" border="1" class="print-friendly" cellspacing="0" cellpadding="2">
        ...
        <thead>
            <tr>
                ...
                <th style="text-align: center; width:10%">CARNET DE IDENTIDAD</th>
                <th style="text-align: center; width:35%">APELLIDO Y NOMBRE</th>
                <th style="text-align: center; width:9%">DIAS TRABAJADOS</th>
                <th style="text-align: center; width:9%">SUELDO PARCIAL</th>
                <th style="text-align: center; width:9%">DESCUENTO</th>
                <th style="text-align: center; width:9%">TOTAL GANADO/LIQUIDO PAGABLE</th>
        ...
        <tbody>
            @php
                ...
                $total = 0;
            @endphp
            @forelse ($spreadsheet->spreadsheetContract as $item)
                <tr>
                    <td>{{ $count }}</td>
                    <td><b>{{ $item->contract->people->ci}}</b></td>
                    <td>                        
                        {{ strtoupper($item->contract->people->last_name1)}} {{ strtoupper($item->contract->people->last_name2)}} {{strtoupper($item->contract->people->first_name)}}
                        @if ($item->paid==1)
                            <label style="background-color: rgb(7, 199, 0);" class="label label-success">PAGADO</label> 
                        @endif
                    </td>
                    <td style="text-align: right">{{ $item->dayWorked }}</td>
                    <td style="text-align: right">{{ number_format($item->payment, 2, '.', '') }}</td>
                    <td style="text-align: right">{{ number_format($item->minuteLateAmount+$item->cantHourAmount, 2, '.', '') }}</td>
                    <td style="text-align: right">{{ number_format($item->liquidPaid, 2, '.', '') }}</td>
                    <td></td>
                                                                            
                </tr>
                @php
                    $count++;                 
                    $total+= number_format($item->liquidPaid, 2, '.', '');                    
                @endphp
            @empty
