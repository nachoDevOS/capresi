            ...
            <tbody>
                @forelse ($data as $item)
                    <tr>
                        ...
                        <td><small>Bs. {{number_format($item->totalPayment, 2, '.', '')}}</small></td>
                        ...
                        <td class="no-sort no-click bread-actions text-right">             
                            @if ($item->status=='pendiente' && $item->deleted_at == NULL)
                                <a title="Aprobar" class="btn btn-sm btn-info" onclick="successItem('{{ route('bonus.generate', ['bonu' => $item->id]) }}')" data-toggle="modal" data-target="#success-modal">
                                    <i class="fa-solid fa-file-csv"></i> Generar</span>
                                </a>
                                <a title="Rechazar" class="btn btn-sm btn-dark" onclick="rechazarItem('{{ route('bonus.rechazar', ['bonu' => $item->id]) }}')" data-toggle="modal" data-target="#rechazar-modal">
                            ...
                            @if (($item->status=='finalizado' || $item->status=='aprobado') && $item->deleted_at == NULL)
                                <a href="{{ route('bonus.print', ['bonu' => $item->id]) }}" target="_blank" title="Imprimir"  class="btn btn-sm btn-dark">
                            ...
                            @if($item->deleted_at == NULL && $item->status != 'rechazado' && $item->status != 'aprobado'&& $item->status != 'finalizado')
                                <a title="Borrar" class="btn btn-sm btn-danger delete" onclick="deleteItem('{{ route('bonus.destroy', ['bonu' => $item->id]) }}')" data-toggle="modal" data-target="#delete-modal">
